...
@functools.partial(jax.jit, static_argnames=['apply_fn'])
def forward_pass_rng(model_params, apply_fn, x, key):
    # Print Statement:
    print('Running Forward Pass...')
...
def loss_function_rng(
    ...
    def forward_pass_rollout(carry, xs):
        ...
        mean, std, values = forward_pass_rng(model_params, apply_fn, model_input, key)
        mean, std, values = jnp.squeeze(mean), jnp.squeeze(std), jnp.squeeze(values)
        # Replay actions:
        log_probability, entropy = jax.vmap(evaluate_action)(mean, std, actions)
        carry = None
...
def train_step_rng(
    ...
    # Print Statement:
    print('Running Train Step...')
    gradient_function = jax.value_and_grad(loss_function_rng)
    loss, gradients = gradient_function(
        model_state.params,
        model_state.apply_fn,
        model_input,
