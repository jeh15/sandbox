...
def main(argv=None):
    ...
    metrics_shared_path = os.path.join(filepath, "metrics_shared.pkl")
    ...
    ax[0].plot(reward_shared, label='MPC Layer Shared', color='limegreen', linewidth=1.5)
    ...
    ax[1].plot(loss_history_ppo, label='MPC Shared', color='darkorange', linewidth=1.5)
    ax[1].plot(loss_history_mpc, label='MPC Layer', color='cornflowerblue', linewidth=1.5)
    ax[1].plot(loss_history_shared, label='MPC Layer Shared', color='limegreen', linewidth=1.5)
    ...
    ax[0].plot(time_vector_shared, reward_shared, label='MPC Layer Shared', color='limegreen', linewidth=1)
    # ax[0].set_xscale('log')
    ax[0].set_title("Total Reward per Episode (avg. across batch)", fontsize=12)
    ...
    ax[1].plot(time_vector_shared, loss_history_shared, label='MPC Layer Shared', color='limegreen', linewidth=1)
    # ax[1].set_xscale('log')
    ax[1].set_title("Loss", fontsize=12)
